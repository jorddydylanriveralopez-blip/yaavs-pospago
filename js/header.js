(() => {
  ...
  function initQuoteLocationPicker() {
    ...
    const isCentralWhatsAppLink = (a) => {
      ...
      // Store landings keep their own Cotiza → WhatsApp with that branch.
      if (a.hasAttribute("data-quote-direct")) return false;
      if (document.body.classList.contains("store-landing")) return false;
      if (a.classList.contains("pospago-stores__wa-btn")) return false;
      if (a.classList.contains("store-float__btn--wa")) return false;
      ...
      const showBranches = () => {
        ...
        renderOptions(
          branches(stateName).map((store) => ({
            ...
            title: store.name,
            sub: [
              store.manager ? `Gerente: ${store.manager}` : "",
              titleCase(store.city),
              store.address,
            ]
              .filter(Boolean)
              .join(" · "),
          ...
          (item) => {
            ...
            if (store.manager) lines.push(`Gerente: ${store.manager}`);
    ...
    const sheetMarkup = (withClose) => `
      ...
      <p class="quote-loc__lead">Elige tu estado, luego la sucursal y el gerente.</p>
